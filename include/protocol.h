...
typedef struct {
	...
	uint8_t remain0;						//byte1: 0 ~ 7 预留0
	uint16_t rpm;							//byte2 ~ 3: 0 ~ 15 电机转速表
	uint32_t odo: 24,						//byte3 ~ 6: 0 ~ 23 总里程
		gear: 3,							//byte3 ~ 6: 24 ~ 26 档位
		remain1: 5;							//byte3 ~ 6: 27 ~ 31 预留1
	...
	uint8_t remain3: 1,						//byte24: 0 预留3
		...
		pControllerErr: 2;					//byte24: 6 ~ 7 P档控制器故障指示灯
	...
	uint16_t remain4;						//byte29 ~ 30: 0 ~ 15 预留4
...
typedef struct{
	...
	uint8_t tripClear: 1,					//byte4: 0 里程小计清零
		remain5: 7;							//byte4: 1 ~ 7 预留5
	uint8_t setBackLight: 4,				//byte5: 0 ~ 3 设置的LCD屏亮度
 ...
 * IMX6 version
 */
#define IMX6VERSION_INFO "V 1.0.0"
...
//update by mcu
#define FRAMETPYES_LEN					3
#define FRAMELENS_LEN					2
#define MCU_PACKET_INFO_TYPES			0x81
#define QT_PACKET_INFO_ACK_TYPES		0x91
#define MCU_PACKET_PROGRESS_TYPES		0x82
#define QT_PACKET_PROGRESS_ACK_TYPES	0x92
#define MCU_PACKET_END_TYPES			0x83
#define QT_PACKET_END_ACK_TYPES			0x93
#define QT_ACK_NG						0x00
#define QT_ACK_OK						0x01

typedef enum {
	NULL_TYPE,
	QT_UPDATA_TYPE,		//0x01---升级QT包帧
	CAR_INFO_TYPE,		//0x02---车身信息帧
	MCU_UPDATA_TYPE,	//0x03---MCU升级包
}FRAME_TYPES;

/*
 * frame sent by ack mcu
 */
typedef struct {
	uint8_t head;
	uint8_t length;
